...
void matrix_mult_slave()
{
   ...
   mres = matrix_create(1, tamA);

   MPI_Scatter(&mres->data[0][0], tamA, MPI_DOUBLE, mres, tamA, MPI_DOUBLE, 0, MPI_COMM_WORLD);
   ...
   //MPI_Gather(&C, tamA, MPI_DOUBLE, mres, tamA, MPI_DOUBLE, 0, MPI_COMM_WORLD);
   matrix_destroy(mres);
   matrix_destroy(B);
   matrix_destroy(C);
}
...
matrix_t *matrix_mult_master(matrix_t *A, matrix_t *B, int ntasks, matrix_t *mres)
{
...
*/
   int tamA = floor(A->rows / ntasks) * A->cols;
   MPI_Bcast(&tamA, 1, MPI_INT, 0, MPI_COMM_WORLD);
   
   //mres = malloc(sizeof(double) * floor(A->rows / ntasks) * A->cols);
   //MPI_Scatter(&A->data[0][0], floor(A->rows / ntasks) * A->cols, MPI_DOUBLE, mres, floor(A->cols / ntasks) * A->cols, MPI_DOUBLE, 0, MPI_COMM_WORLD);
   mres = matrix_create(1, A->cols);
   MPI_Scatter(&A->data[0][0], floor(A->rows / ntasks) * A->cols, MPI_DOUBLE, mres, floor(A->cols / ntasks) * A->cols, MPI_DOUBLE, 0, MPI_COMM_WORLD);

...
/* Allocate memory for particles */
    ...
    double clock_timer = omp_get_wtime();
/* Generate the initial values */
    ...
    printf("%f\n", (double)(omp_get_wtime() - clock_timer));
...
#pragma omp parallel for shared(max_f) private(i) schedule(dynamic)
  for (i=0; i<npart; i++) {
    int j;
    ...
    #pragma omp parallel for reduction(-: fx, fy) shared(rmin, r) private(j, rx, ry, mj) firstprivate(xi, yi)
    for (j=0; j<npart; j++) {
      rx = xi - others[j].x;
      ry = yi - others[j].y;
      mj = others[j].mass;
...
#pragma omp parallel for firstprivate(a0, a1, a2) private(i)
  for (i=0; i<npart; i++) {
    double xi, yi;
    xi	           = particles[i].x;
    yi	           = particles[i].y;
